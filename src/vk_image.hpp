#pragma once

#include "common.hpp"
#include "vk_device.hpp"
#include "vk_buffers.hpp"
#include <vulkan/vulkan_core.h>

namespace coronas_triangle {
    namespace image_creation {
        class ImageTexture {
            public:
            //Initialize without loading image file and/or creating VkImage
            ImageTexture();
            //Load image file and create its VkImage
            ImageTexture(std::string& path, Device& device, VkFormat format);
            ~ImageTexture();
            //loads image from file
            //will add support for other formats but RGBA8 is fine
            void load_image_file(std::string& path);
            //creates VkImage (for fine grained stuff)
            void create(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties);
            //image and its memory
            const VkImage& get_image() const {return _image;}
            const VkDeviceMemory& get_image_memory() const {return _image_memory;}
            const VkImageView& get_image_view() const {return _image_view;}
            private:
            VkImage _image;
            VkDeviceMemory _image_memory;
            VkImageView _image_view;
            int32_t _image_width;
            int32_t _image_height;
            int32_t _image_channels;
            Device& _device;
        };

        void transition_image_layout(Device& device, ImageTexture& image, VkFormat format, VkImageLayout old_layout, VkImageLayout new_layout);
        void copy_buffer_to_image(Buffer& buffer, ImageTexture& image, uint32_t width, uint32_t height);

    }
}
